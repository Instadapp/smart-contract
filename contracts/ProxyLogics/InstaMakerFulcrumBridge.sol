...
contract FulcrumHelper is MakerHelper {
...
contract MakerResolver is FulcrumHelper {
    ...
    function wipe(uint cdpNum, uint _wad) internal returns (uint daiAmt) {
        if (_wad > 0) {
            ...
            // TODO: Replace getCDAIAddress() with iDAI address
...
contract Bridge is FulcrumResolver {

    event LogMakerToFulcrum(uint ethAmt, uint daiAmt);
    event LogFulcrumToMaker(uint ethAmt, uint daiAmt);
...
contract InstaMakerFulcrumBridge is Bridge {
